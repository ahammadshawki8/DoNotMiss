// Popup script - handles the extension popup UI with mock AI-detected tasks

// Mock AI-detected tasks for demo
const MOCK_TASKS = [
  {
    id: 1,
...
async function loadTasks() {
  ...
  // Get tasks from storage, or use mock tasks for demo
  const result = await chrome.storage.local.get('pendingTasks');
  let tasks = result.pendingTasks;
  
  // If no stored tasks, use mock tasks for demo
